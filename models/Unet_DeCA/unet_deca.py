...
class UNetDeCA(nn.Module):
    def __init__(self, n_classes=2, in_channels=5, is_distillation = False):
        super(UNetDeCA, self).__init__()
        ...
        self.channel_att_mlp6 = nn.Sequential(Flatten(), nn.Linear(128, 128 // 8), nn.ReLU(), nn.Linear(128 // 8, 128))
        self.channel_att_mlp7 = nn.Sequential(Flatten(), nn.Linear(64,  64  // 8), nn.ReLU(), nn.Linear(64  // 8, 64 ))
        self.channel_att_mlp8 = nn.Sequential(Flatten(), nn.Linear(32,  32  // 4), nn.ReLU(), nn.Linear(32  // 4, 32 ))
        self.channel_att_mlp9 = nn.Sequential(Flatten(), nn.Linear(16,  16  // 2), nn.ReLU(), nn.Linear(16  // 2, 16 ))
        ...
        # 240 = (128 + 64 + 32 + 16)*2
        self.channel_feature_conv = nn.Conv2d(256, 480, kernel_size=1)

        self.channel_conv6 = nn.Conv2d(256, 128, kernel_size=3, padding=0)
        self.channel_conv7 = nn.Conv2d(128, 64,   kernel_size=3, padding=0)
        self.channel_conv8 = nn.Conv2d(64, 32,   kernel_size=3, padding=0)
        self.channel_conv9 = nn.Conv2d(32, 16,   kernel_size=3, padding=0)

        self.channel_feature_conv = nn.Conv2d(256, 240, kernel_size=1)

        self.channel_conv6 = nn.Conv2d(128, 128, kernel_size=3, padding=0)
        self.channel_conv7 = nn.Conv2d(64, 64,   kernel_size=3, padding=0)
        self.channel_conv8 = nn.Conv2d(32, 32,   kernel_size=3, padding=0)
        self.channel_conv9 = nn.Conv2d(16, 16,   kernel_size=3, padding=0)
    ...
    def forward(self, x):
        ...
        embeddings = self.channel_feature_conv(c5)

        ca_6 = self.channel_conv6(embeddings[:, 0:128,   :, :])
        ca_7 = self.channel_conv7(embeddings[:, 128:192, :, :])
        ca_8 = self.channel_conv8(embeddings[:, 192:224, :, :])
        ca_9 = self.channel_conv9(embeddings[:, 224:240, :, :])

        # ca_6 = self.channel_conv6(embeddings[:, 0:256,   :, :])
        # ca_7 = self.channel_conv7(embeddings[:, 256:384, :, :])
        # ca_8 = self.channel_conv8(embeddings[:, 384:448, :, :])
        # ca_9 = self.channel_conv9(embeddings[:, 448:480, :, :])

        ca_6 = F.avg_pool2d(ca_6, (ca_6.size(2), ca_6.size(3)), stride=(ca_6.size(2), ca_6.size(3)))
        ca_7 = F.avg_pool2d(ca_7, (ca_7.size(2), ca_7.size(3)), stride=(ca_7.size(2), ca_7.size(3)))
        ca_8 = F.avg_pool2d(ca_8, (ca_8.size(2), ca_8.size(3)), stride=(ca_8.size(2), ca_8.size(3)))
        ca_9 = F.avg_pool2d(ca_9, (ca_9.size(2), ca_9.size(3)), stride=(ca_9.size(2), ca_9.size(3)))

        ca_6 = self.channel_att_mlp6(ca_6)
        ca_7 = self.channel_att_mlp7(ca_7)
        ca_8 = self.channel_att_mlp8(ca_8)
        ca_9 = self.channel_att_mlp9(ca_9)
        ...
        ca_6 = F.sigmoid(ca_6).unsqueeze(2).unsqueeze(3).expand_as(c6)
        c6 = ca_6 * c6
        ...
        ca_7 = F.sigmoid(ca_7).unsqueeze(2).unsqueeze(3).expand_as(c7)
        c7 = ca_7 * c7
        ...
        ca_8 = F.sigmoid(ca_8).unsqueeze(2).unsqueeze(3).expand_as(c8)
        c8 = ca_8 * c8
        ...
        ca_9 = F.sigmoid(ca_9).unsqueeze(2).unsqueeze(3).expand_as(c9)
        c9 = ca_9 * c9
